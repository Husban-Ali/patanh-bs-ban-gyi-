...
document.addEventListener('DOMContentLoaded', function() {
  ...
  sendMessageButton.addEventListener('click', async () => {
    ...
    if (message !== '') {
      ...
      try {
        await addDoc(collection(db, 'typescript'), {
          userName: userName,
          message: message,
          timestamp: timestamp
        });
  ...
  const q = query(collection(db, 'typescript'));
  ...
  function displayMessage(userName, message, timestamp) {
    ...
    messageElement.innerHTML = `<p>${message}:<br><br> </p> <strong><i class="fa-solid fa-user font-semibold"></i>${userName} </strong>  ${timeAgo} ago<hr>`;
